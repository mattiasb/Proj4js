...
Proj4js.Proj = Proj4js.Class({
	 ...
	 *
	 * Parameters:
	 *   def - a Proj4- or WKT-definition of the projection
	 */
	initialize: function(def) {
		//check to see if this is a WKT string
		if ((def.indexOf('GEOGCS') >= 0) ||
			(def.indexOf('GEOCCS') >= 0) ||
			(def.indexOf('PROJCS') >= 0) ||
			(def.indexOf('LOCAL_CS') >= 0)) {
            this.parseWKT(def);
            this.deriveConstants();
            this.loadProjCode(this.projName);
            return;
		} else {
			this.loadProjDefinition(def);
		}
	},
	...
	parseDefs: function(proj4) {
		...
		for (var prop=0; prop<paramArray.length; prop++) {
			...
			var paramName = property[0].toLowerCase();
			var paramVal = property[1];
